...
var vSD_FCC7629CBE8A419C84B44B7E51362F16 = {
   vHandle_NAVIGATETO: function(sToForm, oPreviousForm)
   {
     ...
     switch (sToForm)
     {
        case "DF_RAMP":
        {
           SETVALUE("COMMAND","lansa request(run) process(DF_RAMP) partition(dem)");
           SENDKEY(KeyEnter);
           Q_CHECK_CURRENT_FORM("DF_RAMP","Unable to navigate to form DF_RAMP");
        }
           break;
        default:
              alert("Form " + this.vName + " cannot navigate to form " + sToForm);
...
var vSD_230B549A6BEB4773A8AF60BAAC718E04 = {
   vHandle_NAVIGATETO: function(sToForm, oPreviousForm)
   {
     var bReturn = true;
     HIDE_CURRENT_FORM();
   ...
   vName      :  "DF_RAMP",
   vGUID      :  "8810C404CC004A8F851C061A6ED64994",
   vFKC       :    null,
   vFKERTS    :    null,
   vFKEVLF    :    null,
   vFKSND     :    null,
   vFKSEQ     :    null,
   vTargets   :  Array("CUS_KEY","ORD_KEY","PRO_KEY","MAIN"),
   vTYPE      :  "J"
}
;
try { VF_SY420_REGISTER_FORM(vSD_230B549A6BEB4773A8AF60BAAC718E04); }
catch (oE) {window.alert(oE.description + " when loading FORMSCRIPT_5");}
var vSD_CC67332E9F4341F5A9795C9C62AE01AC = {
   ...
   vFKSND     :    null,
   vFKSEQ     :    null,
   vTargets   :  Array("CUS_MAINT","DF_RAMP"),
   vTYPE      :  "J"
}
;
...
var vSD_2DA0E981309B4277BD3BF869E8BDEFCA = {
   ...
   vFKSND     :    null,
   vFKSEQ     :    null,
   vTargets   :  Array("ORD_MAINT","DF_RAMP"),
   vTYPE      :  "J"
}
;
...
var vSD_8958FB625D5945189548166542A127D4 = {
   vHandle_NAVIGATETO: function(sToForm, oPreviousForm)
   {
     ...
     switch (sToForm)
     {
        ...
        case "PRO_MAINT2":
        {
           SETVALUE("KEY",objListManager.NKey1[0]);
           SENDKEY(KeyEnter);
           Q_CHECK_CURRENT_FORM("PRO_MAINT2","Unable to navigate to form PRO_MAINT");
        }
        break;
        default:
              alert("Form " + this.vName + " cannot navigate to form " + sToForm);
   ...
   vFKERTS    : "",
   vFKSND     :  null,
   vTargets   :  Array("DF_RAMP","PRO_MAINT2"),
   vTYPE      :  "D"
}
;
...
var vSD_450069561280409A8C257B8546B3A3A8 = {
   vHandle_ARRIVE: function(oPayload, oPreviousForm)
   {
     var bReturn = true;
     SHOW_CURRENT_FORM(true); /* Show the form in the framework and show VLF buttons */
   ...
   vName      :  "PRO_MAINT2",
   vGUID      :  "6D7BF7816EA540AEAD86B812EADB83B1",
   vOverrideExitJunction : "", /* You may change this property dynamically */
...
"Attention","PA1"),
   vFKSEQ : "0102030405060708091011121314151617181920212223242526272829303132333435363738",
   vFKERTS : "10001000000000100000000000000000000000",
   vFKEVLF : "00000000000000000000000000000000000000",
   vFKSND   :     Array("Enter","F1","F2","F3","F4","F5",
"F6","F7","F8","F9","F10","F11",
"F12","F13","F14","F15","F16","F17",
"F18","F19","F20","F21","F22","F23",
"F24","ClearScreen","PageUp","PageDown","Print","Help",
"Attention","PA1"),
   vTargets   :  Array("DF_RAMP","PRO_KEY"),
   vTYPE      :  "D"
}
;
try { VF_SY420_REGISTER_FORM(vSD_450069561280409A8C257B8546B3A3A8); }
catch (oE) {window.alert(oE.description + " when loading FORMSCRIPT_12");}
function EXEC_026()
{
objNodeGroup.uRampStamp="20170124-140806";
